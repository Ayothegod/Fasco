...
import { confirmOtp } from "../lib/schema";
...
export default function ForgetPassword() {
  ...
  const [form, fields] = useForm({
    lastResult,
    onValidate({ formData }) {
      return parseWithZod(formData, { schema: confirmOtp });
    },
      ...
      <div className="flex mt-20 md:mt-0 w-full sm:max-w-sm md:max-w-3xl border rounded-md overflow-hidden">
        ...
        <div className="w-full md:w-[50%] flex flex-col justify-center px-8 py-4 space-y-4">
          ...
          <Form method="post" className="space-y-4">
            <div>
              <Input placeholder="Confirmation code..." name="otp" />
              {fields.otp.errors ? (
                <p className="text-sm text-red-500 mt-2">{fields.otp.errors}</p>
              ) : null}
            </div>
